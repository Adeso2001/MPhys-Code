...
class CombinedInOut(UIExample):
    def __init__(self, master=None):
        ...
        self.calibration_data = (None, None, None, None, None, None, None, None)
    ...
    def update_ac_output(self):
        if self.ac_mode:
            ...
            self.master.after(1, self.update_ac_output)
    ...
    def create_temperature_indicators(self, parent):
        ...
        for i in range(8):
            ...
            value_label = ttk.Label(temp_frame, text="N/A °K", font=("Segoe UI", 13, "bold"))
    ...
    def update_analog_inputs(self):
        if self.running:
            ...
            for i in range(8):
                ...
                if self.recording:
                    self.record_data.append((timestamp, i, value))
                # Update plot data
                self.input_data[i].append((timestamp, value))
            self.update_plot()
            if self.lockin_enabled:
                self.process_lockin()
            self.master.after(1, self.update_analog_inputs)
    ...
    def load_data(self):
        ...
        if file_path:
            with open(file_path, 'r') as csvfile:
                ...
                for row in reader:
                    timestamp, channel, value = float(row[0]), int(row[1]), float(row[2])
                    self.record_data.append((timestamp, channel, value))
